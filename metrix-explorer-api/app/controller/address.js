...
class AddressController extends Controller {
  async summary() {
    ...
    ctx.body = {
      ...
      mrc20Balances: summary.mrc20Balances.map(item => ({
      ...
      mrc721Balances: summary.mrc721Balances.map(item => ({
        address: item.addressHex.toString('hex'),
        addressHex: item.addressHex.toString('hex'),
        name: item.name,
        symbol: item.symbol,
  ...
  async mrc20TokenBalance() {
    let {ctx} = this
    let {address, token} = ctx.state
    if (token.type !== 'mrc20') {
      ctx.body = {}
    }
    let {name, symbol, decimals, balance, unconfirmed} = await ctx.service.mrc20.getMRC20Balance(address.rawAddresses, token.contractAddress)
    ctx.body = {
      name,
      symbol,
      decimals,
  ...
  async mrc20TokenTransactions() {
    let {ctx} = this
    let {address, token} = ctx.state
    let {totalCount, transactions} = await ctx.service.address.getAddressMRC20TokenTransactions(address.rawAddresses, token)
    ctx.body = {
      totalCount,
      transactions: transactions.map(transaction => ({
        transactionId: transaction.transactionId.toString('hex'),
  ...
  async mrc20TokenMempoolTransactions() {
    let {ctx} = this
    let {address, token} = ctx.state
    let transactions = await ctx.service.address.getAddressMRC20TokenMempoolTransactions(address.rawAddresses, token)
  ...
  async mrc20BalanceHistory() {
    const {Address} = this.app.metrixinfo.lib
    let {ctx} = this
    let tokenAddress = null
    if (ctx.state.token) {
      if (ctx.state.token.type === 'mrc20') {
    ...
    let {totalCount, transactions} = await ctx.service.mrc20.getMRC20BalanceHistory(hexAddresses, tokenAddress)
    ctx.body = {
      totalCount,
