...
class ContractController extends Controller {
  async summary() {
    ...
    ctx.body = {
      ...
      ...summary.type === 'mrc20' ? {
        mrc20: {
          name: summary.mrc20.name,
          symbol: summary.mrc20.symbol,
          decimals: summary.mrc20.decimals,
          totalSupply: summary.mrc20.totalSupply.toString(),
          version: summary.mrc20.version,
          holders: summary.mrc20.holders,
          transactions: summary.mrc20.transactions
        }
      } : {},
      ...summary.type === 'mrc721' ? {
        mrc721: {
          name: summary.mrc721.name,
          symbol: summary.mrc721.symbol,
          totalSupply: summary.mrc721.totalSupply.toString()
        }
      } : {},
  ...
  async mrc20BalanceHistory() {
    ...
    let {totalCount, transactions} = await ctx.service.mrc20.getMRC20BalanceHistory([ctx.state.contract.contractAddress], tokenAddress)
    ctx.body = {
      totalCount,
      transactions: transactions.map(tx => ({
        id: tx.id.toString('hex'),
