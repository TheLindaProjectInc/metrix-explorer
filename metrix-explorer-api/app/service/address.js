...
class AddressService extends Service {
  async getAddressSummary(addressIds, p2pkhAddressIds, rawAddresses) {
    const {Address} = this.ctx.app.metrixinfo.lib
    ...
    let [
      ...
      mrc20Service.getAllMRC20Balances(hexAddresses),
      mrc721Service.getAllMRC721Balances(hexAddresses),
  ...
  async getAddressMRC20TokenTransactionCount(rawAddresses, token) {
    const {Address, Solidity} = this.app.metrixinfo.lib
    const TransferABI = Solidity.mrc20ABIs.find(abi => abi.name === 'Transfer')
  ...
  async getAddressMRC20TokenTransactions(rawAddresses, token) {
    const {Address, Solidity} = this.app.metrixinfo.lib
    const TransferABI = Solidity.mrc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {sql} = this.ctx.helper
    ...
    let topicAddresses = rawAddresses
      .filter(address => address.type === Address.PAY_TO_PUBLIC_KEY_HASH)
      .map(address => Buffer.concat([Buffer.alloc(12), address.data]))
    let totalCount = await this.getAddressMRC20TokenTransactionCount(rawAddresses, token)
    let transactions = await db.query(sql`
      SELECT
      ...
      INNER JOIN mrc20 ON mrc20.contract_address = log.address
  ...
  async getAddressMRC20TokenMempoolTransactions(rawAddresses, token) {
        ...
        },
        {
          ...
          include: [{
            ...
            include: [{
              ...
              as: 'contract',
              required: true,
              where: {address: token.contractAddress, type: 'mrc20'},
              attributes: []
            }]
          }]
        },
