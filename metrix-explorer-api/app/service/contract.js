...
class ContractService extends Service {
  async getContractAddresses(list) {
    const {Address} = this.app.metrixinfo.lib
  ...
  async getContractSummary(contractAddress, addressIds) {
    const {Contract, Mrc20: MRC20, Mrc20Statistics: MRC20Statistics, Mrc721: MRC721} = this.ctx.model
    const {balance: balanceService, mrc20: mrc20Service, mrc721: mrc721Service} = this.ctx.service
    ...
    let [
      ...
      mrc20Service.getAllMRC20Balances([contractAddress]),
      mrc721Service.getAllMRC721Balances([contractAddress]),
    ...
    return {
      ...
      ...contract.type === 'mrc20' ? {
        mrc20: {
          name: contract.mrc20.name,
          symbol: contract.mrc20.symbol,
          decimals: contract.mrc20.decimals,
          totalSupply: contract.mrc20.totalSupply,
          version: contract.mrc20.version,
          holders: contract.mrc20.statistics.holders,
          transactions: contract.mrc20.statistics.transactions
        }
      } : {},
      ...contract.type === 'mrc721' ? {
        mrc721: {
          name: contract.mrc721.name,
          symbol: contract.mrc721.symbol,
          totalSupply: contract.mrc721.totalSupply
        }
      } : {},
  ...
  async getContractBasicTransactions(contractAddress) {
    const {Address, OutputScript} = this.app.metrixinfo.lib
    const {
