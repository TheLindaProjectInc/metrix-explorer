...
class MiscService extends Service {
  async classify(id) {
    ...
    const {Block, Transaction, Contract, Mrc20: MRC20, where, fn, literal} = db
    ...
    let mrc20Results = (await MRC20.findAll({
      where: {
        [$or]: [
      ...
      transaction
    })).map(mrc20 => mrc20.contractAddress)
    if (mrc20Results.length === 0) {
      mrc20Results = (await MRC20.findAll({
        where: {
          [$or]: [
    ...
    if (mrc20Results.length) {
      let [{addressHex}] = await db.query(sql`
        SELECT contract.address_string AS address, contract.address AS addressHex FROM (
          SELECT contract_address FROM mrc20_statistics
          WHERE contract_address IN ${mrc20Results}
          ORDER BY transactions DESC LIMIT 1
        ) mrc20_balance
        INNER JOIN contract ON contract.address = mrc20_balance.contract_address
