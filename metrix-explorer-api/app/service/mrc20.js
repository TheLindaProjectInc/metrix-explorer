...
class MRC20Service extends Service {
  async listMRC20Tokens() {
    const db = this.ctx.model
    const {Mrc20Statistics: MRC20Statistics} = db
    const {sql} = this.ctx.helper
    const {gt: $gt} = this.app.Sequelize.Op
    let {limit, offset} = this.ctx.state.pagination

    let totalCount = await MRC20Statistics.count({
    ...
    let list = await db.query(sql`
      SELECT
        ...
        mrc20.name AS name, mrc20.symbol AS symbol, mrc20.decimals AS decimals, mrc20.total_supply AS totalSupply,
        mrc20.version AS version,
        list.holders AS holders,
        list.transactions AS transactions
      FROM (
        SELECT contract_address, holders, transactions FROM mrc20_statistics
        WHERE transactions > 0
        ORDER BY transactions DESC
        LIMIT ${offset}, ${limit}
      ) list
      INNER JOIN mrc20 USING (contract_address)
  ...
  async getAllMRC20Balances(hexAddresses) {
    if (hexAddresses.length === 0) {
      return []
    }
    const {OutputScript, Solidity} = this.app.metrixinfo.lib
    const transferABI = Solidity.mrc20ABIs.find(abi => abi.name === 'transfer')
    const {
      Address, TransactionOutput,
      Contract, EvmReceipt: EVMReceipt, Mrc20: MRC20, Mrc20Balance: MRC20Balance,
      where, col
    } = this.ctx.model
    const {in: $in} = this.app.Sequelize.Op
    let list = await MRC20.findAll({
    ...
    let mapping = new Map(list.map(item => [
      item.contract.addressString,
      {
        ...
        symbol: item.symbol,
        decimals: item.decimals,
        balance: item.contract.mrc20Balances.map(({balance}) => balance).reduce((x, y) => x + y),
        unconfirmed: {
          received: 0n,
          sent: 0n
        }
      ...
      if (byteCode.length === 68
        ...
        if (mapping.has(item.output.address.contract.addressString)) {
          ...
          data = {
            ...
            name: item.output.address.contract.mrc20.name,
            symbol: item.output.address.contract.mrc20.symbol,
            decimals: item.output.address.contract.mrc20.decimals,
            balance: 0n,
            unconfirmed: {
              received: 0n,
              sent: 0n
  ...
  async getMRC20Balance(rawAddresses, tokenAddress) {
    const {Address: RawAddress, OutputScript, Solidity} = this.app.metrixinfo.lib
    const transferABI = Solidity.mrc20ABIs.find(abi => abi.name === 'transfer')
    const {
      Address, TransactionOutput,
      Contract, EvmReceipt: EVMReceipt, Mrc20: MRC20, Mrc20Balance: MRC20Balance,
    ...
    let token = await MRC20.findOne({
      where: {contractAddress: tokenAddress},
      attributes: ['name', 'symbol', 'decimals'],
      transaction: this.ctx.state.transaction
    })
    let list = await MRC20Balance.findAll({
  ...
  async getMRC20BalanceHistory(addresses, tokenAddress) {
    const TransferABI = this.app.metrixinfo.lib.Solidity.mrc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {sql} = this.ctx.helper
    const {
      Header, Transaction,
      EvmReceipt: EVMReceipt, EvmReceiptLog: EVMReceiptLog,
      Contract, Mrc20: MRC20, Mrc20Balance: MRC20Balance,
      literal
    } = db
    ...
    let [{totalCount}] = await db.query(sql`
      ...
      FROM evm_receipt receipt, evm_receipt_log log, mrc20
      WHERE receipt._id = log.receipt_id AND log.address = mrc20.contract_address AND ${{raw: logFilter}}
    `, {type: db.QueryTypes.SELECT, transaction: this.ctx.state.transaction})
    if (totalCount === 0) {
      return {totalCount: 0, transactions: []}
    }
    let ids = (await db.query(sql`
      ...
      INNER JOIN (
        SELECT DISTINCT(receipt.transaction_id) AS id FROM evm_receipt receipt, evm_receipt_log log, mrc20
        WHERE receipt._id = log.receipt_id AND log.address = mrc20.contract_address AND ${{raw: logFilter}}
    ...
    if (list.length > 0) {
      let intialBalanceList = await MRC20Balance.findAll({
        where: {
      ...
      for (let log of logs) {
        ...
        if (item) {
          ...
          result.tokens.push({
            ...
            name: log.mrc20.name.toString(),
            symbol: log.mrc20.symbol.toString(),
            decimals: log.mrc20.decimals,
            amount: delta
          })
        }
      }
  ...
  async getAllMRC20TokenTransactions() {
    const TransferABI = this.app.metrixinfo.lib.Solidity.mrc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {sql} = this.ctx.helper
    let {limit, offset, reversed = true} = this.ctx.state.pagination
    ...
    let [{totalCount}] = await db.query(sql`
      ...
      FROM mrc20, evm_receipt_log log
      WHERE mrc20.contract_address = log.address AND log.topic1 = ${TransferABI.id}
    `, {type: db.QueryTypes.SELECT, transaction: this.ctx.state.transaction})
    let transactions = await db.query(sql`
      ...
      FROM (
        SELECT log._id AS _id FROM mrc20, evm_receipt_log log
        WHERE mrc20.contract_address = log.address AND log.topic1 = ${TransferABI.id}
  ...
  async getMRC20TokenTransactions(contractAddress) {
    const TransferABI = this.app.metrixinfo.lib.Solidity.mrc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
  ...
  async getMRC20TokenRichList(contractAddress) {
    const db = this.ctx.model
    const {Mrc20Balance: MRC20Balance} = db
    const {ne: $ne} = this.app.Sequelize.Op
    let {limit, offset} = this.ctx.state.pagination

    let totalCount = await MRC20Balance.count({
  ...
  async updateMRC20Statistics() {
    const TransferABI = this.app.metrixinfo.lib.Solidity.mrc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {Mrc20: MRC20, Mrc20Statistics: MRC20Statistics} = db
    const {sql} = this.ctx.helper
    let transaction = await db.transaction()
    try {
      let result = (await MRC20.findAll({attributes: ['contractAddress'], transaction})).map(
        ({contractAddress}) => ({contractAddress, holders: 0, transactions: 0})
      )
      let balanceResults = await db.query(sql`
        SELECT contract_address AS contractAddress, COUNT(*) AS count FROM mrc20_balance
      ...
      await db.query(sql`DELETE FROM mrc20_statistics`, {transaction})
      await MRC20Statistics.bulkCreate(result, {validate: false, transaction, logging: false})
