...
class TransactionService extends Service {
  async getTransaction(id) {
    const {
      ...
      Contract, Mrc20: MRC20, Mrc721: MRC721,
      where, col
    } = this.ctx.model
    const {in: $in} = this.app.Sequelize.Op
    const {Address: RawAddress} = this.app.metrixinfo.lib
        ...
        if (output.evmReceipt) {
          ...
          outputObject.evmReceipt.logs = eventLogs.filter(log => log.receiptId === output.evmReceipt._id).map(log => ({
            ...
            ...log.mrc20 ? {
              mrc20: {
                name: log.mrc20.name,
                symbol: log.mrc20.symbol,
                decimals: log.mrc20.decimals
              }
            } : {},
            ...log.mrc721 ? {
              mrc721: {
                name: log.mrc721.name,
                symbol: log.mrc721.symbol
              }
            } : {}
          }))
        }
  ...
  async getRawTransaction(id) {
    ...
    const {Transaction: RawTransaction, Input, Output, OutputScript} = this.app.metrixinfo.lib
  ...
  async transformTransaction(transaction, {brief = false} = {}) {
    ...
    let [mrc20TokenTransfers, mrc20TokenUnconfirmedTransfers, mrc721TokenTransfers] = await Promise.all([
      this.transformMRC20Transfers(transaction.outputs),
      confirmations === 0 ? this.transformMRC20UnconfirmedTransfers(transaction.outputs) : undefined,
      this.transformMRC721Transfers(transaction.outputs)
    ])
    ...
    return {
      ...
      mrc20TokenTransfers,
      mrc20TokenUnconfirmedTransfers,
      mrc721TokenTransfers
    }
  }

  transformInput(input, index, transaction, {brief}) {
    const {InputScript, OutputScript} = this.app.metrixinfo.lib
  ...
  transformOutput(output, index, {brief}) {
    const {OutputScript} = this.app.metrixinfo.lib
  ...
  async transformMRC20Transfers(outputs) {
    const TransferABI = this.app.metrixinfo.lib.Solidity.mrc20ABIs.find(abi => abi.name === 'Transfer')
    let result = []
    for (let output of outputs) {
      if (output.evmReceipt) {
        for (let {addressHex, topics, data, mrc20} of output.evmReceipt.logs) {
          if (mrc20 && topics.length === 3 && Buffer.compare(topics[0], TransferABI.id) === 0 && data.length === 32) {
  ...
  async transformMRC721Transfers(outputs) {
    const TransferABI = this.app.metrixinfo.lib.Solidity.mrc20ABIs.find(abi => abi.name === 'Transfer')
    let result = []
    for (let output of outputs) {
      if (output.evmReceipt) {
        for (let {addressHex, topics, mrc721} of output.evmReceipt.logs) {
          if (mrc721 && topics.length === 4 && Buffer.compare(topics[0], TransferABI.id) === 0) {
            let [from, to] = await this.ctx.service.contract.transformHexAddresses([topics[1].slice(12), topics[2].slice(12)])
            result.push({
              address: addressHex.toString('hex'),
              addressHex: addressHex.toString('hex'),
              name: mrc721.name,
              symbol: mrc721.symbol,
              ...from && typeof from === 'object' ? {from: from.hex.toString('hex'), fromHex: from.hex.toString('hex')} : {from},
              ...to && typeof to === 'object' ? {to: to.hex.toString('hex'), toHex: to.hex.toString('hex')} : {to},
  ...
  async getContractTransaction(receiptId) {
    const {Address: RawAddress, OutputScript} = this.app.metrixinfo.lib
    const {
