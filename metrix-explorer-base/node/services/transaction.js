...
class TransactionService extends Service {
  ...
  async _processBlock(block) {
    ...
    if (this.#synced) {
      ...
      await this.#Transaction.bulkCreate(txs, {validate: false, ignoreDuplicates:true})
    }
  ...
  async processTxos(transactions) {
    ...
    await Promise.all([
      this.#TransactionOutput.bulkCreate(outputTxos, {validate: false, ignoreDuplicates:true}),
      this.#TransactionInput.bulkCreate(inputTxos, {validate: false, ignoreDuplicates:true}),
      ...mappings.length ? [
        this.#db.query(sql`
