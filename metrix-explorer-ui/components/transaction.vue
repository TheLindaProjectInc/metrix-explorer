<template>
  <div class="columns is-multiline transaction-item">
          ...
          <div class="output-script">
            ...
            <div v-if="output.scriptPubKey.asm">
              ...
              <code class="value" :class="{script: contractInfo[index]}"
                ...
                -->{{ output.scriptPubKey.asm | metrix-script }}<!--
              --></code>
            </div>
    ...
    <template v-for="({addressHex, name, symbol, decimals, from, fromHex, to, toHex, value}, index) in mrc20TokenTransfers">
      <div class="column is-full flex-full"></div>
      <AttributeInjector
        class="column collapse token-transfer-list"
        :class="{
          'first-item': index === 0,
          'last-item': index === mrc20TokenTransfers.length - 1
        }">
        <div class="is-clearfix">
    ...
    <template v-for="({addressHex, name, symbol, from, fromHex, to, toHex, tokenId}, index) in mrc721TokenTransfers">
      <div class="column is-full flex-full"></div>
      <AttributeInjector
        class="column collapse token-transfer-list"
        :class="{
          'first-item': index === 0,
          'last-item': index === mrc721TokenTransfers.length - 1
        }">
        <div class="is-clearfix">
          <AddressLink v-if="from" :address="fromHex || from" class="is-pulled-left" :highlight="highlightAddress" />
          <template v-else>{{ $t('contract.token.mint_tokens') }}</template>
    ...
    <div class="column is-full has-text-right collapse-bottom" v-if="fees !== '0'">
      <template v-if="fees > 0">
        {{ $t('transaction.fee') }} <span class="amount fee">{{ fees | metrix }} MRX</span>
      </template>
      <template v-else-if="fees < 0">
        {{ $t('transaction.reward') }} <span class="amount fee">{{ -fees | metrix }} MRX</span>
      </template>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      mrc20TokenTransfers() {
        return this.transaction.mrc20TokenTransfers
      },
      mrc721TokenTransfers() {
        return this.transaction.mrc721TokenTransfers
      },
      contractInfo() {
